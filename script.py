...
def build_swagger(auth_eps, tables, rpcs, edge_fns, base_url, anon_key, app_url):
    ...
    for ep in auth_eps:
        ...
        fp = "/auth/v1" + p
        body = (
            {"type": "object", "properties": {bk: {"type": "string"} for bk in ep["body_keys"]}}
            if ep["body_keys"]
            else None
        )
    ...
    for t in tables:
        ...
        paths[p] = {
            ...
            "post": _op(
                "post", p, f"Insert {t}", "rest", body={"type": "object", "additionalProperties": True}, anon_key=k
            ),
            "patch": _op(
                "patch",
                p,
    ...
    for r in rpcs:
        ...
        paths[p] = {
            "post": _op(
                "post", p, f"RPC: {r}", "rpc", body={"type": "object", "additionalProperties": True}, anon_key=k
            )
        }
...
def main():
    ...
    parser.add_argument("--skip-download", action="store_true", help="Skips download if assets already exist")
    parser.add_argument("--no-test", action="store_true", help="Skips endpoint testing")
    parser.add_argument("--methods", default="get,post", help="Methods to test (default: get,post)")
