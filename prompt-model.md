...
Contrato reutilizavel derivado deste repositorio de referencia (Supabase Recon Analyzer). Use este arquivo para orientar um agente a gerar ou adaptar qualquer projeto (Python ou outra linguagem) com o mesmo padrao de engenharia: DDD, hexagonal, TDD, qualidade, DX e documentacao.
...
Objetivo final: o novo repositorio deve "parecer" este projeto em arquitetura, qualidade e operacao — nao copiar o negocio de recon Supabase.
...
Imports limpos: `from domain...`, `from application...`.

## 4. Qualidade e DX

Gates conceituais obrigatorios: lint/format, type check, dead code, test+coverage, security, hooks, Make (`app-install`, `app-lint`, `app-test`, `app-security`, `app-clean`, `app-setup`, `app-pre-commit-run`).

Orquestrador central unico chamado pelo Makefile (neste repo: `app/scripts/operations/clean_workspace.py`).

Para Python novo: Ruff, mypy strict, vulture, pytest-cov 100% branch, bandit, pip-audit, pre-commit, commitlint, Makefile.

## 5. Logging semantico

API unica do tipo `log_event(logger, level, event, **fields)`.

Eventos nomeados e estaveis (`*.started|finished|failed|skipped_*`). Caminho feliz: ~3 linhas INFO. Sem dump de payload/body em INFO. Redact de URLs/segredos. `exc_info` so em DEBUG. Silenciar HTTP clients em WARNING+.

Detalhes: `docs/engineering-logging.md`.

## 6. Documentacao obrigatoria

`docs/arquitetura.md`, `docs/structure.md`, `docs/engineering-<lang>.md`, `docs/engineering-logging.md`, `AGENTS.md`, `README.md`, `.env.example`.

## 7. Protocolo de adaptacao

1. Ler dominio/requisitos e linguagem alvo.
2. Listar entidades, use cases, ports e adapters.
3. Criar arvore e impor regras de dependencia.
4. Configurar build, `.env.example`, Makefile e orquestrador.
...
9. Rodar gates ate verde.
10. Garantir Conventional Commits e ausencia de segredos.
...
- Baixar fail-under sem decisao explicita.
...
- Outra linguagem: escolha equivalentes idiomaticos mantendo os MESMOS gates conceituais.
...
5) Nao copiar stack de dominio alheio a menos que o novo dominio peca algo equivalente.
